...
<head>
    ...
    button:hover {
        background-color:rgb(180, 210, 243);
    }
...
<body>
    ...
    <button type="submit">Submit Procuct in</button>
</form>
    </div>
</body>
</html>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO productin (ProductCode, DateTime, Quantity, UnitPrice, TotalPrice)
            VALUES (?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isidd", $productCode, $dateTime, $quantity, $unitPrice, $totalPrice);

    if ($stmt->execute()) {
        echo "<script>alert('Product-in record added successfully!');</script>";
    } else {
